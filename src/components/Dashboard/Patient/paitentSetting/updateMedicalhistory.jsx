import axios from "../../../../../src/utils/axios"; // Adjust path as needed
...
const UpdateMedicalHistory = ({ id }) => {
    ...
    const [vaccinationHistory, setVaccinationHistory] = useState([{ vaccine: "", date: "" }]);
    ...
    const fetchVaccines = async () => {
        try {
            const res = await axios.get("/landingPage/vaccines");
    ...
    const fetchExistingHistory = async () => {
        try {
            const res = await axios.get(`/medical-history/${id}`);
            const data = res.data;
            setAllergies(data.allergies?.join(", ") || "");
            setChronicDiseases(data.chronicDiseases?.join(", ") || "");
            setMedications(data.medications || [{ name: "", dosage: "", frequency: "" }]);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await axios.post(`/patient/medical-history/update/${id}`, {
                allergies: allergies.split(",").map((a) => a.trim()),
                chronicDiseases: chronicDiseases.split(",").map((d) => d.trim()),
                medications,
                vaccinationHistory,
            });
    ...
    if (!id) {
        return <div className="text-red-600 p-4">❌ No patient ID provided.</div>;
    }

    return (
        <div className="p-6 max-w-4xl mt-10 mx-auto bg-white shadow rounded-xl">
            <h2 className="text-3xl font-bold mb-6 text-blue-700">📝 Update Medical History</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {/* Vaccination History */}
                <div>
                    <label className="block font-medium mb-2">Vaccination History</label>
                    {vaccinationHistory.map((v, i) => (
                        <div key={i} className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                            <select
                                value={v.vaccine}
                                onChange={(e) => updateVaccine(i, "vaccine", e.target.value)}
                                className="border px-3 py-2 rounded"
                            >
                    ...
                    <button
                        type="button"
                        onClick={() => setVaccinationHistory([...vaccinationHistory, { vaccine: "", date: "" }])}
                        className="text-sm text-blue-600 hover:underline"
                    >
                        ➕ Add Vaccination
                    </button>
