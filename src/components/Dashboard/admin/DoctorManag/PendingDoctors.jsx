...
const DoctorApprovalDashboard = () => {
    ...
    const fetchUnapprovedDoctors = async () => {
        try {
            setLoading(true);
            const res = await axios.get("http://localhost:10/api/admin/unapproved-doctors");
            setDoctors(res.data);
        } catch (err) {
    ...
    const fetchDoctorDetails = async (userId) => {
        try {
            const res = await axios.get(`http://localhost:10/api/landingPage/doctor/${userId}`);
    ...
    const approveDoctor = async (userId) => {
        try {
            await axios.put(`http://localhost:10/api/admin/approve-doctor/${userId}`);
    ...
    const removeDoctor = async (userId) => {
        try {
            await axios.delete(`http://localhost:10/api/remove-doctor/${userId}`);
    ...
    return (
        <div className="p-6">
            <h2 className="text-3xl font-bold mb-6 text-gray-800">Pending Approval</h2>
                ...
                <div className="fixed inset-0 z-50 flex justify-center items-center backdrop-blur-sm overflow-y-auto">
                    <div className="bg-white border-gray-100 border-2 p-6 rounded-md w-96 relative">
                        ...
                        <div className="space-y-2 text-gray-700 text-sm">
                            <p>
                                <strong>Name:</strong> {selectedDoctor.user?.name || "N/A"}
                            </p>
                            <p>
                                <strong>Email:</strong> {selectedDoctor.user?.email || "N/A"}
                            </p>
                            <p>
                            ...
                            <p>
                                ...
                                {selectedDoctor.isApproved ? "Approved" : "Not Approved"}
                            </p>
                        </div>
